...
var drag = {
	...
	keyboardDragDrop : function(objEvent)
	{
		...
		var arChoices = ['Main Basket', 'Fruit Basket', 'Vegetable Basket'];
		...
		var objItem = drag.objCurrent;
			...
			if (iKey == 32)
			{
				...
				// Set ARIA properties
				drag.objCurrent.setAttribute('draggable', 'true');
		...
		switch (iKey)
		{
			...
			case 9  : // Tab
				...
				for (iCounter=0; iCounter<drag.arTargets.length; iCounter++)
				{
					...
					drag.objCurrent.setAttribute('draggable', 'false');
	...
	dropObject : function(strTarget)
	{
		...
		if (strTarget.length > 0)
		{
			...
			if (objBands.length === 0)
			{
				...
				objItem.appendChild(document.createTextNode(''));// removed 'None' from node
...
function init ()
{
	...
	for (iCounter=0; iCounter<objItems.length; iCounter++)
	{
		...
		objItems[iCounter].setAttribute('draggable', 'false');
